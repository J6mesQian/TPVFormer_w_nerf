...
@HEADS.register_module()
class TPVAggregator_binary_occ(BaseModule):
    def __init__(
        self, tpv_h, tpv_w, tpv_z, nbr_classes=1, 
        in_dims=64, hidden_dims=128, out_dims=None,
        scale_h=2, scale_w=2, scale_z=2, use_checkpoint=True
    ):
        super().__init__()
        ...
        self.occ_classifier = nn.Sequential(
            nn.Linear(out_dims, nbr_classes)
        )
        self.classes = nbr_classes
        self.use_checkpoint = use_checkpoint
    ...
    def forward(self, tpv_list):
        ...
        # [bsz, embed_dim, H, W, Z] -> [bsz, embed_dim, H*W*Z]
        fused = fused.flatten(2)
        fused = fused.permute(0, 2, 1) #[bsz, H*W*Z, embed_dim]
        if self.use_checkpoint:
            predict_voxel_feat = torch.utils.checkpoint.checkpoint(self.feat_decoder, fused)  #[bsz, H*W*Z, embed_dim]
            predict_occ_label = torch.utils.checkpoint.checkpoint(self.occ_classifier, predict_voxel_feat)  #[bsz, H*W*Z, 1]
            # appied sigmoid
            predict_occ_label = torch.sigmoid(predict_occ_label)
        else:
            predict_voxel_feat = self.feat_decoder(fused)
            predict_occ_label = self.occ_classifier(predict_voxel_feat)
            # appied sigmoid
            predict_occ_label = torch.sigmoid(predict_occ_label)
        # predict_voxel_feat = predict_voxel_feat.permute(0, 2, 1) #[bsz, embed_dim, H*W*Z]
        predict_occ_label = predict_occ_label.permute(0, 2, 1) #[bsz, 1, H*W*Z]
        
        import ipdb
        #ipdb.set_trace()
        
        # reshape back
        # predict_voxel_feat = predict_voxel_feat.reshape(bs, self.out_dims, self.scale_w*self.tpv_w, self.scale_h*self.tpv_h, self.scale_z*self.tpv_z)
        predict_occ_label = predict_occ_label.reshape(bs, self.classes, self.scale_w*self.tpv_w, self.scale_h*self.tpv_h, self.scale_z*self.tpv_z)
        
        return predict_occ_label
    
@HEADS.register_module()
class TPVAggregator_w_NeRF(BaseModule):
    def __init__(
        self, tpv_h, tpv_w, tpv_z, nbr_classes=1, 
        in_dims=64, hidden_dims=128, out_dims=None,
        scale_h=2, scale_w=2, scale_z=2, use_checkpoint=True
    ...
    def forward(self, tpv_list):
        ...
        if self.use_checkpoint:
            predict_voxel_feat = torch.utils.checkpoint.checkpoint(self.feat_decoder, fused)  #[bsz, H*W*Z, embed_dim]
            predict_occ_label = torch.utils.checkpoint.checkpoint(self.occ_classifier, fused)  #[bsz, H*W*Z, 1]
            # appied sigmoid
            predict_occ_label = torch.sigmoid(predict_occ_label)
        else:
            predict_voxel_feat = self.feat_decoder(fused)
            predict_occ_label = self.occ_classifier(fused)
            # appied sigmoid
            predict_occ_label = torch.sigmoid(predict_occ_label)
        predict_voxel_feat = predict_voxel_feat.permute(0, 2, 1) #[bsz, embed_dim, H*W*Z]
        predict_occ_label = predict_occ_label.permute(0, 2, 1) #[bsz, 1, H*W*Z]
        ...
        return predict_voxel_feat, predict_occ_label