...
def main(local_rank, args):
    ...
    if dist.get_rank() == 0:
        ...
        run_name = f"{current_time}-TPVformer_w_NeRF_binary_occ_exp0_bce"
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, (imgs, img_metas, train_nerf_vox_feat, train_occ_label) in enumerate(train_dataset_loader):
            ...
            outputs_vox_occ_label = my_model(img=imgs, img_metas=img_metas)
            ...
            if i_iter % print_freq == 0 and dist.get_rank() == 0:
                ...
                logger.info('[TRAIN] Epoch %d Iter %5d/%d: Occ_Loss: %.4f, LR: %.6f, Grad_Norm: %.4f, Step_Time: %.4f, Data_Time: %.4f'%(epoch, i_iter, len(train_dataset_loader), occ_loss_train.item(), lr, grad_norm, time_e - time_s, data_time_e - data_time_s))
            if dist.get_rank() == 0:
        ...
        with torch.no_grad():
            for i_iter_val, (imgs, img_metas, val_nerf_vox_feat, val_occ_label) in enumerate(val_dataset_loader):
                ...
                outputs_vox_occ_label_val = my_model(img=imgs, img_metas=img_metas)
                # Step 1: calculate loss for voxel occ label
                bce_loss_func = nn.BCELoss()
                focal_loss_func = FocalLoss()
                ...
                if i_iter_val % print_freq == 0 and dist.get_rank() == 0:
                    logger.info('[EVAL] Epoch %d Iter %5d: Occ Loss: %.4f'%(epoch, i_iter_val, occ_loss_val.item()))
                if dist.get_rank() == 0:
